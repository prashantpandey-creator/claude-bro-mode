---
name: claude-bro-mode
description: Install Claude Bro Mode (Orchestrator-First methodology + hyped-homie-who-codes persona) into the current project and the user's global Claude config. Use when the user says "install claude bro mode", "set up the methodology", "wire the pre-flight hook", or "install the persona".
tools: Bash, Read, Write, Edit
---

# Claude Bro Mode — Installer

Install both halves of Claude Bro Mode:

1. **Orchestrator-First methodology** (per project): AGENTS.md rules, doc_path_audit tool, SessionStart hook, settings patch
2. **Bro persona** (per user, global): output style + optional CLAUDE.md block
...
- "install bro mode" / "install claude bro mode" → both
- "install the orchestrator" / "wire the hook" → methodology only
- "install the persona" / "install the bro voice" → persona only

### Step 1 — Locate the source repo

```bash
ls ~/projects/claude-bro-mode/ 2>/dev/null || \
ls ~/claude-bro-mode/ 2>/dev/null || \
...
```bash
git clone https://github.com/prashantpandey-creator/claude-bro-mode ~/claude-bro-mode
```
...
cp "$REPO/persona/bro.md" ~/.claude/output-styles/bro.md
```
...
```bash
# Check whether a bro block already exists to avoid duplicates
grep -q "name: bro" ~/.claude/CLAUDE.md 2>/dev/null && \
  echo "already in ~/.claude/CLAUDE.md, skipping" || \
  cat "$REPO/persona/bro.md" >> ~/.claude/CLAUDE.md
```
...
`~/.claude/output-styles/bro.md`, then rename the file and run
...
- Persona: that `/output-style bro` activates the voice, and they can rename
...
- Next step for verification: start a fresh Claude Code session and confirm both
  ...
  reads hyped-but-competent from the first response
